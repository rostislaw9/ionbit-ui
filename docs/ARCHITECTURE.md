...
| Trace (border beam)                          | CSS keyframes              | `conic-gradient` masked to the border ring; `@property`-registered angle animated once per lap |
...
| Trace     | Replaced by a static accent ring.                         |
