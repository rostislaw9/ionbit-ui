...
- **Open Graph image layout.** `apps/docs/public/og-image.png` is now
  generated by `apps/docs/scripts/generate-og-image.py`
  (`yarn workspace @ionbit-ui/docs og:image`), matching the site's
  typography (SF Pro semibold with tight tracking, SF Mono wordmark).
  Content is optically centered with doubled side margins so it
  survives squarer social-preview crops.
